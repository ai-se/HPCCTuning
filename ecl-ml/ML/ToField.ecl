...
EXPORT ToField(dIn,dOut,idfield='',datafields=''):=MACRO
  ...
  dOut:=NORMALIZE(dIn,%iNumberOfFields%,TRANSFORM(ML.Types.NumericField,SELF.id:=LEFT.#EXPAND(%'foundidfield'%),SELF.number:=COUNTER;SELF.value:=CHOOSE(#EXPAND(%'normlist'%))));
